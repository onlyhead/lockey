    ...
    namespace ec {
        ...
        class ECEngine {
          public:
            ...
            virtual bool verify(const std::vector<uint8_t> &hash, const std::vector<uint8_t> &signature,
        ...
        class P384Engine : public ECEngine {
          private:
            P384 curve_;

          public:
            KeyPair generate_keypair() override {
                auto private_key = curve_.generate_private_key();
                auto public_key_point = curve_.compute_public_key(private_key);

                KeyPair result;
                result.private_key = private_key;
                result.public_key = public_key_point;
                return result;
            }
            ...
            bool verify(const std::vector<uint8_t> &hash, const std::vector<uint8_t> &signature,
                ...
                if (signature.size() != 96)
                    return false; // r + s = 48 + 48 bytes

                Signature sig;
                sig.r.assign(signature.begin(), signature.begin() + 48);
                sig.s.assign(signature.begin() + 48, signature.end());
        ...
        class P521Engine : public ECEngine {
          private:
            P521 curve_;
          ...
          public:
            ...
            bool verify(const std::vector<uint8_t> &hash, const std::vector<uint8_t> &signature,
                ...
                if (signature.size() != 132)
                    return false; // r + s = 66 + 66 bytes

                Signature sig;
                sig.r.assign(signature.begin(), signature.begin() + 66);
                sig.s.assign(signature.begin() + 66, signature.end());
